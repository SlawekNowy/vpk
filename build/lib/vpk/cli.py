...
def make_argparser():
    ...
    info.add_argument("-ch","--chunk",type=int,help="Operate on individual chunk.")
...
def print_file_list(pak, match_filter=None,chunk_filter=None, include_details=False):
    for path, metadata in pak.read_index_iter():
        if match_filter and not match_filter(path):
            continue
        if chunk_filter and not chunk_filter(pak,path):
            continue
...
def extract_files(pak, match_filter,chunk_filter, outdir, makedir=False):
...
def pipe_files(pak, match_filter,chunk_filter):
    for filepath in pak:
        if match_filter and not match_filter(filepath):
            continue

        if chunk_filter and not chunk_filter(pak,filepath):
            continue
...
def run(args):
    ...
    chunk_filter = None
    if args.chunk is not None: #FYI chunk CAN be 0
        #chunk is given.
        #first check if we're dealing with infile.
        #This is expensive...
        chunk_id = args.chunk
        has_chunks = False
        if args.chunk <0 or args.chunk >= 0x7fff:
            raise argparse.ArgumentError("Invalid ID of the chunk %s. Accepted values: 0 - 32766.",args.chunk)
        for filename in pak:
            metadata = pak.get_file_meta(filename)
        ...
        if not has_chunks:
            raise argparse.ArgumentError("Given --chunk argument when file is not split into chunks!")

        def _chunk_filter(pak:vpk.VPK,path:str):
            return pak.get_file_meta(path)['archive_index'] == chunk_id
    ...
    if args.list or args.listall:
        print_file_list(pak, path_filter,chunk_filter, args.listall)
    elif args.pipe_output:
        pipe_files(pak, path_filter,chunk_filter)
    elif args.test:
        print_verifcation(pak)
    elif args.out_location:
        extract_files(pak, path_filter,chunk_filter, args.out_location, args.makedir)
    else:
        print_header(pak)
